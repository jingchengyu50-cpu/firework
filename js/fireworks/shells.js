...
function startSequence() {
	if (isFirstSeq) {
		...
		if (fireworksPerformance.canLaunchShell()) {
			new Shell(crysanthemumShell(shellSizeSelector())).launch(0.5, 0.35);
		}
		return 4200;
	}
	...
	if (!IS_MOBILE && randomValue < 0.05 && Date.now() - seqSmallBarrage.lastCalled > seqSmallBarrage.cooldown) {
		return seqSmallBarrage();
	}
	if (!IS_MOBILE && randomValue < 0.08) {
		return seqPyramid();
	}
	if (randomValue < 0.72 && !IS_HEADER) {
