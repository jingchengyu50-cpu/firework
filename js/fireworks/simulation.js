...
function update(frameTime, lag) {
	...
	fireworksPerformance.trimParticles();
...
function render(speed, width, height) {
	...
	const trailAlpha = store.state.config.longExposure ? 0.0025 : 0.16 * speed;
	trailsContext.fillStyle = `rgba(6, 8, 24, ${trailAlpha})`;
	...
	while (BurstFlash.active.length) {
		...
		burstGradient.addColorStop(0.024, "rgba(255, 248, 232, 0.85)");
		burstGradient.addColorStop(0.125, "rgba(255, 210, 140, 0.14)");
		burstGradient.addColorStop(0.32, "rgba(200, 170, 120, 0.08)");
		burstGradient.addColorStop(1, "rgba(120, 100, 80, 0)");
...
const currentSkyColor = { r: BASE_SKY_COLOR.r, g: BASE_SKY_COLOR.g, b: BASE_SKY_COLOR.b };
const targetSkyColor = { r: BASE_SKY_COLOR.r, g: BASE_SKY_COLOR.g, b: BASE_SKY_COLOR.b };

function colorSky(speed) {
	const maxSkySaturation = skyLightingSelector() * 10;
	const maxStarCount = 500;
	let totalStarCount = 0;

	targetSkyColor.r = BASE_SKY_COLOR.r;
	targetSkyColor.g = BASE_SKY_COLOR.g;
	targetSkyColor.b = BASE_SKY_COLOR.b;
...
class Shell {
	...
	launch(position, launchHeight) {
		if (!fireworksPerformance.canLaunchShell()) {
			return;
		}

		fireworksPerformance.onShellLaunch();
	...
	burst(x, y) {
		fireworksPerformance.onShellBurst();
...
const Star = {
	...
	add(x, y, color, angle, speed, life, speedOffsetX, speedOffsetY, size = 3) {
		fireworksPerformance.trimStars(true);
		const instance = fireworksPerformance.stampParticle(this._pool.pop() || this._new());
...
const Spark = {
	...
	add(x, y, color, angle, speed, life) {
		fireworksPerformance.trimSparks(true);
		const instance = fireworksPerformance.stampParticle(this._pool.pop() || this._new());
		instance.x = x;
		instance.y = y;
		instance.prevX = x;
