...
const Stage = (function StageFactory(window, document, Ticker) {
	...
	function getDevicePixelRatio(context) {
		...
		if (window.FireworksMobile && window.FireworksMobile.getDpr) {
			return window.FireworksMobile.getDpr();
		}

		var backingStoreRatio = context.backingStorePixelRatio || 1;
		var deviceRatio = (window.devicePixelRatio || 1) / backingStoreRatio;
	...
	Stage.prototype.resize = function resize(width, height, nextDpr) {
		if (typeof nextDpr === "number" && nextDpr > 0) {
			this.dpr = nextDpr;
		}
